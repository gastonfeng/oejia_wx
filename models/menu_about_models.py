...
MENU_ACTION_OPTION = ACTION_OPTION + [('wx.action.act_url', u'跳转链接')]


class menu_item_base(models.AbstractModel):
    _name = 'wx.menu.item.base'

    menu_id = fields.Many2one('wx.menu', string=u'所属微信菜单', required=True, ondelete='cascade')
    sequence = fields.Integer('Sequence', help="sequence")
    name = fields.Char(u'子菜单', )
    action = fields.Reference(string=u'动作', selection=MENU_ACTION_OPTION)
...
class wx_menu(models.Model):
    ...
    left = fields.Char(u'左菜单')
    left_action = fields.Reference(string=u'动作', selection=MENU_ACTION_OPTION)
    middle = fields.Char(u'中菜单')
    middle_action = fields.Reference(string=u'动作', selection=MENU_ACTION_OPTION)
    right = fields.Char(u'右菜单')
    right_action = fields.Reference(string=u'动作', selection=MENU_ACTION_OPTION)
    sequence = fields.Integer('Sequence', help="sequence")

    mtype = fields.Selection([(1, u'公众号'), (2, u'企业号')], string=u'类型', default=1)
